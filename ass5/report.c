...
{
  sem_wait(&(bank->workerLock));
  bank->numWorkersUnready--;
  if (bank->numWorkersUnready == 0) {
    
  } else {
    sem_post(&(bank->workerLock));
    sem_wait(&(bank->readyForNextDay));
    return 0;
  }
  Report *rpt = bank->report;
  ...
  if (rpt->numReports >= MAX_NUM_REPORTS) {
      ...
      for(int i = 0; i < numWorkers - 1; i++) {
        sem_post(&(bank->readyForNextDay));
      }
      bank->numWorkersUnready = workerNum;
      // We've run out of report storage for the bank
      return -1;
  }
  ...
  bank->numWorkersUnready = numWorkers;
  sem_post(&(bank->workerLock));
  return err;
}
